...
This checkout is the consolidated `korentomas/agent-delegate-sprint-2026` fork.
`origin` points to this fork; `upstream` points to the original
`mpodeley/agent-delegate-sprint-2026` repository. Work from the fork’s `main`.
The user authorized integrating the existing branches here. Rules reserving
Matías’s `main` and submission decisions refer to upstream and authorship, not
to permission to maintain this fork. Do not submit the report on the authors’ behalf.
Keep the two `codex/*` PR source branches while upstream PRs #2 and #3 remain open;
retired variants are preserved as `archive/2026-09-13/*` tags.
...
Also: the English visual lab (`web/`, deployed at https://korentomas.github.io/agent-delegate-sprint-2026/), a source-grounded casebook (`data/grounded_cases.json`, `docs/grounded-evidence.md`), a Spanish plain-language summary (`docs/explicacion-sencilla.md`), and the larger research program (`docs/research-program.md`).
